...
{
    public partial class 退货信息查询 : Form
    {
        public 退货信息查询()
        {
            InitializeComponent();
        ...
        private void 退货信息查询_Load(object sender, EventArgs e)
        {
            ...
            com.Connection = conn;
            com.CommandText = "select * from ReturnList";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (r.IsMatch(s))
            {
                if (s.Length == 10)
                {
                    com.CommandText = "select OLNum from ReturnList where OLNum='" + s + "'";
                }
                else if(s.Length == 13)
                {
                    com.CommandText = "select OLNum from ReturnList where OLNum in ( select OLNum from OrderList where ISBN='" + s + "')";
            ...
            {
                com.CommandText = "select OLNum from ReturnList where OLNum in ( select OLNum from OrderList where BookName='" + s + "')";
            }
            SqlDataReader reader = com.ExecuteReader();
